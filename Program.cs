...
namespace _RPG_Fighting_Game
{
    ...
    class Store
    {
        ...
        public void DisplayStore(Player _Player)
        {
            ...
            {
                Console.WriteLine("{0} - {1} - {2} Gold - {3} - {4}x Quantity", StoreInventory.IndexOf(StoreInventory[i]) + 1, StoreInventory[i].name, StoreInventory[i].goldValue,
                ...
                else if (choice == "5")
                {
                    ...
                    while (Player.currentHealth > 0 && Goblin.health > 0)
                    {
                        ...
                        else if (battleChoice == "3")
                        {
                            ...
                            for (int i = 0; i < PlayerInventory.PlayerBag.Count; i++)
                            {
                                if (inventoryChoice == "1")
                                {
                                    if (Player.gold >= PlayerInventory.PlayerBag[i].goldValue)
                                    {
